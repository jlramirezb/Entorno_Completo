...
class ContenedorCompuesto extends Container {
  ...
  showQuestionModal() {
    ...
    if ((this.auxNodo.firstElementChild.nodeName == "INPUT" && this.auxNodo.firstElementChild.type === 'text') && this.auxNodo.firstElementChild.type != undefined) {
      ...
      // mathFieldElement.addEventListener('click', () => {
      //   mathVirtualKeyboard.layouts = this.keyBoardProfile;
      //   mathVirtualKeyboard.visible = true;
      // });    //  mathFieldElement.focus()
  ...
  fillTheModal(originalNodo) {
    ...
    if (!hasRadioInput) {
      ...
      try {
        this.auxNodo.querySelector('input').value = this.originalNodo.querySelector('input').value
      } catch (e) {
        this.auxNodo.querySelector('math-field').value = this.originalNodo.querySelector('math-field').value


      }

      //this.auxNodo.layouts = this.keyBoardProfile;
  ...
  expressionValidation() {
    ...
    if (noSlashText === "") {
      ...
      if (Array.isArray(this.validacion)) {
        processedCondition = this.validacion.map((element) => element.replace(/(?:left|right)/g, ""))
        ...
        processedCondition = this.validacion.replace(/(?:left|right)/g, "")
