class SumConstantArtifact extends EngineMaker {
  ...
  validation() {
    ...
    if (this.definition.conditions?.multiplyCurves) {
      conditions = this.definition.conditions?.multiplyCurves

           //valida si elecciona las curvas correctas
      let curveValidates = this.validationCurves({
   ...
   /*    console.log(">>", this.definition.conditions?.asymptotes.length === this.pointsValidate.length);
      ...
      if (this.definition.conditions?.asymptotes.length === this.pointsValidate.length && this.definition.visiblexSlice 
        && this.definition.visibleAsyntote) {

        let puntos = this.definition.conditions?.asymptotes.every((tall) => {

          const math = this.pointsValidate.findIndex(
            (point) => {
              
              return (gInterPoint(point.Y(), tall.y, 0.2)
                &&
                gInterPoint(point.X(), tall.x, 0.2))
            }
          ) != -1
        ...
        if (puntos) {
          ...
          // this.functionsCurves()
          this.alert.createSuccessAlert('Excelente')
        } else {
          this.alert.createErrorAlert('Mal')
        }

      } else {
        this.alert.createWarningAlert('nada')
      }
