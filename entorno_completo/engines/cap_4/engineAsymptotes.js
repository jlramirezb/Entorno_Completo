function mainAsymptotes(defBoards, allDef) {
  ...
  function asymptotesValidation(asymptotes, positions, noise = 0.5) {
  
    asymptotes.sort((p1, p2) => { return p1.X() - p2.X(); });
    positions.sort((p1, p2) => { return p1.x - p2.x; });
    return positions.every((p, i) => {
      //si esta la propiedad y evalua signo y coordenda y, en caso de que no este evalua solo el signo
      const response =  gInterPoint(asymptotes[i].X(), p.x, noise) && asymptotes[i].name().toLowerCase().replace(' ', '') === p.name.toLowerCase().replace(' ', '') && (p.y ? gInterPoint(asymptotes[i].Y(), p.y, noise): true )
      return response
    });
