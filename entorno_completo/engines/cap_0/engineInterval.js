...
function generation(def) {
  ...
  for (let index of Object.keys(def)) {
    ...
    if (
      !defDefault?.conditions?.board && clone.querySelector('.allButtons')
    ) {
      clone.querySelector('.allButtons').style.display = 'none';
    }
...
function defBoard(def) {
  const allColors = {
    ...
    arrow: { color: '#217e9d' },
    curves: 'black',
    colorLines: ['black', '#ff8000', 'blue', 'red', 'green']
  };
  ...
  for (let index of Object.keys(def)) {
    ...
    if (def2.preDefPoint) {
      defPointDefault(def[index], board);
    }
    if (def2.preDefPar) {
      defParDefault(def[index], board);
    }

    if (def2.preDefPoint || def2.preDefPar || !def2.conditions?.board) {
...
function defNewPoint(def, e, target) {
  ...
  const { points } = def.colors;
  //evita que se solapen los puntos
  const elementIn = def.board.getAllUnderMouse();
  if (elementIn.findIndex((item) => item.elType === 'point') != -1 || !def.conditions?.board) {
    return;
  }
  def.change = true;
  ...
  if (!def.preDefPoint) {
    ...
    if (def.state == 'pointA') {
      const { strokeColor, fillColor } = points.open;
    ...
    if (def.state == 'pointC') {
      const { color, strokeColor, fillColor } = points.closed;
      point = def.board.create('point', [x, 0], {
        name: '',
        color,
        strokeColor,
...
function defPointDefault(def, board) {
  const size = siteMode == 'desktop' ? 6 : 6;
  const { points } = def.colors;
  const offset = [
    siteMode == 'desktop' ? 0.2 : 0.15,
  ...
  for (let element of def.preDefPoint) {

    const colorsElement = points[element[2] ? 'open' : 'closed'];
    const { color, fillColor, strokeColor } = colorsElement;
    ...
    board.create('point', [element[0], element[1]], {
      ...
      fixed: true,
      size,
      /*  fillColor: element[2] ? 'white' : '#FFFFFF', */
    });
...
function defParDefault(def, board) {
  const size = siteMode == 'desktop' ? 6 : 6;
  const { points } = def.colors;
  const offset = [
    siteMode == 'desktop' ? 0.2 : 0.15,
  ...
  for (let element of def.preDefPar) {

    const colorsElement1 = points[element[0][2] == 0 ? 'open' : 'closed'];
    let p1 = board.create('point', [element[0][0], element[0][1]], {
      name: '', //element[0][2] ? "(" : "[",
      fixed: true,
      size,
      color: colorsElement1.color,
      fillColor: colorsElement1.fillColor,
      strokeColor: colorsElement1.strokeColor,
      visible: true,
      label: {
        fontSize: 15,
        offset: [0, 1],
    ...
    const colorsElement2 = points[element[1][2] == 0 ? 'open' : 'closed'];
    let p2 = board.create('point', [element[1][0], element[1][1]], {
      name: '', //element[1][2] ? ")" : "]",
      fixed: true,
      size,
      color: colorsElement2.color,
      fillColor: colorsElement2.fillColor,
      visible: true,
      label: {
        fontSize: 15,
        offset: [0, 1],
...
function defConection(def, onePoint, twoPoint, target) {
  ...
  const { points } = def.colors;
  const elementColor = points.closed;
  def.statusValidate = false;
  def.change = true;
  if (
  ...
  if (twoPoint == null) {
    ...
    if (twoPoint[0].id === onePoint[0].id) {
      twoPoint[0].setAttribute({
        strokeColor: elementColor.strokeColor,
        size,
        label: { color: "black" },
      });
      twoPoint = null;
