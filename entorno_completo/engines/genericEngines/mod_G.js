  ...
  return params.points.map((p, i) => {
    ...
    if (Array.isArray(p)) {
      point = params.board.create('point', [p[0], p[1]], {
        ...
        fillcolor: typeof p[5] == 'string' ? p[5] : p[5] ? 'white' : 'black',
        strokeColor: typeof p[5] == 'string' ? p[5] : 'black',
...
function gBoard(def, boardSelect, id, style, callback = {}, i, params) {
  ...
  const reflectionAxieCurves = gCurveDefault({
    def,
    board,
    id,
    curves: simetriAxiesCurves,
    ...params,
  });
  ...
  def.reflectionAxie = {
    ...
    axieBCurve: reflectionAxieCurves[0],
  };
  ...
  //crea curvas con una expresion matemática, ejemplo: sin(x)
  if (boardSelect.expressionCurve) {
    resultObj.expressionCurve = gExpressionCurve({
      def,
      board,
      expression: boardSelect.expressionCurve,
      ...params
    });
  }

  if (boardSelect.inputs) {

    resultObj.inputs = createInputs({
      board,

      inputs: boardSelect.inputs
    });
...
function gInterPoint(value, compare, noise = 0.1, minDecimal = 2) {
...
function gAddPoint(def, board, color = 'black', anchor, coords) {
  const remove = [];


  if (!coords) {

    coords = board.getUsrCoordsOfMouse();
...
function gOperationCurves(
  ...
  let { curve_1, curve_2, type, board } = params;
  const accuracy = params.def.accuracy ?? 2;
  const domOfCurves = (points) => {
    return points.reduce((accum, item) => {
      let x = item.usrCoords[1].toFixed(accuracy);
      let y = item.usrCoords[2].toFixed(accuracy);
      if (!accum[x]) { accum[x] = y; }
      return accum;
    }, {});

  };
  let domCurve_1 = domOfCurves(curve_1.points);
  let domCurve_2 = domOfCurves(curve_2.points);
  let resultCurv = board.create('curve', ['x',
    (x) => {
      let coord = x.toFixed(accuracy);
      let point_1 = parseFloat(domCurve_1[coord]);
      let point_2 = parseFloat(domCurve_2[coord]);

      let operation;
      if (type == "1") {
        operation = point_1 + point_2;
      }
      if (type == "2") {
        operation = point_1 * point_2;
      }
...
function gAsymptotes(
  ...
  if (asymptotes.length > 0) {
    return asymptotes.map((asyn) => {
      ...
      asymptote.validate = asyn?.validate ?? false;
...
function gStyleMath(pointLatex) {
  ...
  /* pongo los chorkucts con un intervalo minimo para evitar que el susuario ponga cosas que no ueire */
  pointLatex.inlineShortcutTimeout = 0.1;
  /* linpia el foco de los artefactos en general */
...
function gClearInputLatex(latex) {
  const baseText = latex.replace(/\\/g, '');
  const commontsRegexs = {
    'right.': ')',
    'ight\\)': ')',
    '\\^{}': '',
    'left\\(': '(',
  ...
  Object.keys(commontsRegexs).forEach(reg => {
    result = result.replace(new RegExp(reg, 'g'), commontsRegexs[reg]);
  });
...
//selector
class CreateSelectsPoints {

  constructor(callback, parent, defAll, modeInsert = 'after') {
    this.parent = parent;
    this.value = null;
    this.modeInsert = modeInsert;
    this.selectMain = document.createElement('div');
    this.defAll = defAll;

    this.ul = document.createElement('ul');
    this.colorBase = this.createObtion(null, this.selectMain, null, { color: 'black' });

    this.selectMain.classList.add('buttonAux', 'button-marg');
    this.selectMain.setAttribute('title', 'Selector de caminos');
    this.ul.classList.add('available-options', 'displayOptions');

    this.initSelector(callback);

  };

  initSelector = (mainCallback) => {
    this.defAll.forEach(((def, i) => {
      const { callback, data, value, style, content } = def;
      this.createObtion(callback ?? mainCallback, this.ul, value ?? i, style, data, content);
    }));

    this.selectMain.addEventListener('click', () => {
      const elementoDesplegable = this.selectMain.querySelector('.available-options');
      elementoDesplegable.classList.toggle('displayOptions');
    });

    this.selectMain.appendChild(this.ul);

    if (this.modeInsert == 'before') {
      //this.parent.insertBefore(li, this.selectMain.firstChild);
      this.parent.insertBefore(this.selectMain, this.parent.firstChild);
      //  this.parent.appendChild(this.selectMain);
    } else if (this.modeInsert == 'after') {
      this.parent.appendChild(this.selectMain);
  ...
  setColor(element, color) {
    element.style.setProperty('--roadColor', color ?? 'black');
  };

  resetColor() {
    this.value = null;
    this.setColor(this.colorBase.childNodes[0], null);
  };

  createObtion = (callback, parent, value = 'default', style, data, content) => {
    ...
    if (content && typeof content === 'string') {
      ...
      span.classList.add('punto-color', 'color');
    };

    li.dataset.value = value;
    li.classList.add('available-option');

    li.addEventListener('click', (e) => {
      this.value = value;
      this.setColor(this.colorBase.childNodes[0], color);
      if (typeof callback === 'function') {
        callback(li.dataset.value, data);
      };

    });

    if (color) {
      this.setColor(span, color);
    };
...
//Funcion para crear inpust de mahtlive en el board
function createInputs(params) {
  const { inputs, board, } = params;
  return inputs.map((input) => {
    const { x, y, value, style, valid = false } = input;
    const disable = ((value && !style?.disabled) || (!value && style?.disabled === true));
    const newInput = board.create(
      "text",
      [
...
//recibe un objeto con do propiedades, la expresion matematica y los estilos de la curva
function gExpressionCurve(params) {
  const paramsDefault = {
    board: null,
    expression: "x",
    interval: [],
    ...params,
    curveStyle: { strokeColor: "black", strokeWidth: 2, ...params.curveStyle },
  };
  if (!paramsDefault?.board) {
    console.warn("!!!!board no definido, en gExpresionCurve");
    return null;
  }
  return paramsDefault.board.create('functiongraph',
    [paramsDefault.expression, ...paramsDefault.interval], paramsDefault.curveStyle);
