...
function PintaSeleccionP1(position, def, defBoards, Pregunta){
    ...
    let artefact = 'artifact_'+ position;
    //let kk = position*2-1
    ...
    //console.log(def)
    const desireddef = {};
    desireddef[artefact] = def[artefact];
    def = desireddef;
    //console.log(def);
    //def.artefact_1.datadefault = def.artefact_1.datadefault.slice((position-1)*2,(position-1)*2+2);*/
    
    let div = document.querySelector('.elementEtw');
    //console.log(div);
    div.setAttribute('data-artifact', artefact);
    div.setAttribute('data-board', newPropertyName);
        //div.id = artefact;
